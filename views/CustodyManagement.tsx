...
const CustodyManagement: React.FC<CustodyProps> = ({ 
  ...
  const currentHoldersData = useMemo(() => {
    ...
    return report.filter(r => (filterEmployeeId === 'ALL' || r.empId === filterEmployeeId) && 
                              (r.empName.toLowerCase().includes(q) || r.itemName.toLowerCase().includes(q) || r.itemCode.toLowerCase().includes(q)));
  }, [custodies, items, employees, filterEmployeeId, filterSearch]);

  const debouncedFilterSearch = useDebounced(filterSearch, 300);

  const archiveData = useMemo(() => {
    const q = debouncedFilterSearch.toLowerCase();
    return custodies.filter(c => {
      const item = items.find(i => i.id === c.itemId);
      const matchesSearch = item?.name.toLowerCase().includes(q) || item?.code.toLowerCase().includes(q) || c.docNumber.toLowerCase().includes(q);
      const matchesEmp = filterEmployeeId === 'ALL' || c.employeeId === filterEmployeeId;
      return matchesSearch && matchesEmp;
    });
  }, [custodies, debouncedFilterSearch, filterEmployeeId, items]);
