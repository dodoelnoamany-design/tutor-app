...
const ItemCoding: React.FC<ItemCodingProps> = ({ items, setItems, units, movements, currentUser }) => {
  ...
  const debouncedSearchShelf = useDebounced(searchShelf, 300);
  const debouncedSearchBox = useDebounced(searchBox, 300);

  const filteredItems = useMemo(() => {
    return items.filter(it => {
      const matchText = it.name.toLowerCase().includes(debouncedSearchTerm.toLowerCase()) || it.code.toLowerCase().includes(debouncedSearchTerm.toLowerCase());
      const matchShelf = !debouncedSearchShelf || it.shelfNumber.toLowerCase().includes(debouncedSearchShelf.toLowerCase());
      const matchBox = !debouncedSearchBox || it.boxNumber.toLowerCase().includes(debouncedSearchBox.toLowerCase());
      return matchText && matchShelf && matchBox;
    });
  }, [items, debouncedSearchTerm, debouncedSearchShelf, debouncedSearchBox]);
